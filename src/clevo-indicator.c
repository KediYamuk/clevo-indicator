...
struct timespec delta = {1 /*secs*/, 0 /*nanosecs*/};
...
static int main_ec_worker(void) {
    ...
    while (share_info->exit == 0) {
        ...
        close(io_fd);
        // auto EC
        // printf("auto_duty = %d, prev_duty = %d, next_duty= %d\n", share_info->auto_duty, share_info->manual_prev_fan_duty, share_info->manual_next_fan_duty );
...
static gboolean ui_update(gpointer user_data) {
    ...
    // sprintf(icon_name, "/usr/local/share/clevo2-indicator.png");
